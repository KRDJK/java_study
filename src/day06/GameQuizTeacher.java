...
public class GameQuizTeacher {
    public static void main(String[] args) {
        ...
        int success = 0; // 성공적으로 장전되었을 때에 카운트가 올라감.
        while (success < bulletNum) {
            int position = (int) (Math.random() * magazine.length);
            if (!magazine[position]) { // 이미 그 자리가 true라면 동작하지 않음.
        ...
        while (true) {
            /*
                ...
                : 랜덤 인덱스 (0~5) 생성하여 총알이 있는 위치(true)가 결정되면 죽는다.
                2. 격발처리 : 탄창배열을 조회해서 true 값인지 확인
                3. 사망 판정 : 해당 격발 위치에 총알이 있었으면 사망처리,
                             없었으면 생존처리
                3-1. 사망 처리 : players 배열에서 죽은 플레이어 이름 삭제
                                탄창 배열에서 격발위치 값 false로 조정.
                4. 턴 조정 : 지금 격발한 플레이어가 마지막 사람이면 다음 턴의 인덱스는 0번 or +1번.
